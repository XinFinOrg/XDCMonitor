...
export default function () {
  ...
  if (txType === 'simple_transfer') {
    ...
    response = http.post(`${BASE_URL}${ENDPOINTS.TRIGGER_TEST_TX}`, JSON.stringify(payload), {
    ...
    response = http.post(`${BASE_URL}${ENDPOINTS.TRIGGER_TEST_CONTRACT}`, JSON.stringify(payload), {
      headers: { 'Content-Type': 'application/json' },
    });
    ...
    contractProcessingTime.add(new Date().getTime() - startTime, {
      chainId: chainId,
      network: chain.name,
      type: txType,
    });
