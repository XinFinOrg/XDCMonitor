...
import { SECURITY_METRICS, SeverityLevel, VulnerabilityType, XDC_SECURITY_CONFIG } from '@common/constants/security';
import axios, { AxiosRequestConfig } from 'axios';
import * as fs from 'fs';
import * as path from 'path';
import * as https from 'https';

// Use the constants from the shared constants file
const SECURITY_CONFIG = {
  // Default report path
  DEFAULT_REPORT_PATH: path.join(process.cwd(), 'reports', 'security'),
...
// Standard API methods to test access
const TEST_METHODS = {
  admin: 'listAccounts',
  debug: 'metrics',
...
export class NetworkScannerService {
  ...
  constructor(
    ...
    this.options = {
      ...
      concurrency: SECURITY_CONFIG.DEFAULT_CONCURRENCY,
      reportPath: SECURITY_CONFIG.DEFAULT_REPORT_PATH
    };
  ...
  private async checkExposedMethods(endpoint: string, vulnerabilities: Vulnerability[]): Promise<void> {
    ...
    for (const api of XDC_SECURITY_CONFIG.RESTRICTED_APIS) {
      try {
        // Try a method from this API namespace
        const method = `${api}_${TEST_METHODS[api]}`;
