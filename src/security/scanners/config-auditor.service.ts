...
export class ConfigAuditorService {
  ...
  private initializeSecurityRules() {
    ...
    this.securityRules.push({
      ...
      check: config => {
        ...
        return (
          config.rpc.http &&
          (config.rpc.http.addr === '0.0.0.0' || config.rpc.http.addr === '*' || !config.rpc.http.addr)
        );
      },
    ...
    this.securityRules.push({
      ...
      check: config => {
        ...
        return (
          config.rpc.http.cors === '*' || (Array.isArray(config.rpc.http.cors) && config.rpc.http.cors.includes('*'))
        );
      },
  ...
  async auditConfigDir(configDir: string): Promise<AuditResult[]> {
    ...
    const files = fs
      ...
      .filter(
        file => file.endsWith('.json') || file.endsWith('.yaml') || file.endsWith('.yml') || file.endsWith('.toml'),
      );
  ...
  private recordAuditMetrics(results: AuditResult[]): void {
    try {
      ...
      const criticalFindings = results.reduce(
        (count, result) => count + result.findings.filter(f => f.severity === SeverityLevel.CRITICAL).length,
        0,
      );

      const highFindings = results.reduce(
        (count, result) => count + result.findings.filter(f => f.severity === SeverityLevel.HIGH).length,
        0,
      );

      const mediumFindings = results.reduce(
        (count, result) => count + result.findings.filter(f => f.severity === SeverityLevel.MEDIUM).length,
        0,
      );

      const lowFindings = results.reduce(
        (count, result) => count + result.findings.filter(f => f.severity === SeverityLevel.LOW).length,
        0,
      );
