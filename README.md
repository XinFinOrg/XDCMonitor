...
This workflow runs on every push to the main/master branches and on pull requests to these branches. It includes:

- **Testing**: Runs linting and unit tests
- **Building**: Builds the application and creates a Docker image
- **Publishing**: Pushes the Docker image to GitHub Container Registry
- **Deployment**: Deploys to production (when configured)

### Docker Compose Deployment

A separate workflow for deploying using Docker Compose:

- Can be triggered manually from GitHub Actions interface
- Supports staging and production environments
- Securely manages environment variables using GitHub Secrets
- Uses SSH to deploy to your server

### Security Scanning

A security-focused workflow that:

- Runs weekly and on package changes
- Performs npm audit and Snyk vulnerability scans
- Scans Docker images with Trivy for container vulnerabilities
- Auto-approves and merges minor and patch Dependabot updates

### Automatic Dependency Updates

The repository is configured with Dependabot to:
...
- Auto-merge safe updates (configurable)

### Setting Up the CI/CD Pipeline

1. Add the following secrets to your GitHub repository:

   - `SSH_PRIVATE_KEY` - SSH key for deployment to servers
   - `SSH_HOST` - Hostname for deployment
   ...
   - `DEPLOY_PATH` - Path where the application should be deployed
   - `RPC_URL`, `WS_URL`, etc. - Your application environment variables

2. Optional: Set up Snyk for advanced security scanning:

   - Create a Snyk account at https://snyk.io/
   - Add your `SNYK_TOKEN` to GitHub secrets

3. For the full CI/CD pipeline to work, ensure your repository has access to:

   - GitHub Packages (for storing Docker images)
   - GitHub Actions (for running workflows)

4. For deployment to work correctly, ensure your server:
   - Has Docker and Docker Compose installed
   - Has appropriate network access for pulling images
   - Has sufficient permissions for the deployment user
